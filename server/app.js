...
db.serialize(function() {
  if(!exists) {
    db.run("CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, name TEXT, firstname TEXT, job_title TEXT, twitter TEXT, website TEXT)");
...
router.get('/people', function(req, res){
  ...
  let db = new sqlite3.Database(filename);
  let users = [];
  db.each("SELECT users.name, users.firstname, job_title, GROUP_CONCAT(skills.name) as skills FROM users LEFT JOIN users_has_skills ON users_has_skills.user_id = users.id LEFT JOIN skills ON skills.id = users_has_skills.skill_id GROUP BY users.id", function(err, row) {
      let skills = [];
      if (row.skills) {
        skills = row.skills.split(',');
      }

      users.push({name: row.name, firstname: row.firstname, jobTitle: row.job_title, picture: '/assets/images/user.png', skills: skills});
  }, function(){
